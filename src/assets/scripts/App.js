...
window.debugMapTransitions = () => {
  const debug = () => {
    ...
    views &&
      views.forEach((element) => (element.style.borderTop = '1px dotted'));
    ...
    anchors &&
      anchors.forEach((element, index) => {
        ...
        if (mapBoxHandler.views.hasOwnProperty(viewName)) {
          const view = mapBoxHandler.getViewParameters(
            mapBoxHandler.views[viewName]
          );
          ...
          element.innerHTML =
            viewName === mapBoxHandler.labels.displacer
